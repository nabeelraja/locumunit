...
# :first_in sets how long it takes before the job is first run. In this case, it is run immediately
	...
	send_event('target', { value:s.cell('C',11), max:s.cell('C',4), title:"Clinic Target #{s.cell('C',4).to_i}" })
	send_event('cc1', { value:s.cell('C',8) })
	send_event('cc2', { value:s.cell('C',9) })
	send_event('cc3', { value:s.cell('C',10) })
	send_event('total_leads', { value:s.cell('C',11) })
	send_event('taxis_booked', { value:s.cell('F',11) })
	send_event('letters_dispatched', { value:s.cell('C',18) })
